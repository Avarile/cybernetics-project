...
"""
Google OAuth views for the main web app (``/auth/google/`` and
``/auth/google/callback/``).

The initiate view stores the frontend host, a random ``state`` (CSRF protection) and
``next_path`` in the session, then redirects to Google. The callback runs
...
class GoogleOauthInitiateEndpoint(View):
    """Start the Google OAuth flow for the main web app."""

    def get(self, request):
        """Save host/``state``/``next_path`` in the session and redirect to the Google authorize URL."""
