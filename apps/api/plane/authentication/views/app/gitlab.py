...
"""
GitLab OAuth views for the main web app (``/auth/gitlab/`` and
``/auth/gitlab/callback/``).

The initiate view stores the frontend host, a random ``state`` (CSRF protection) and
``next_path`` in the session, then redirects to GitLab. The callback runs
...
class GitLabOauthInitiateEndpoint(View):
    """Start the GitLab OAuth flow for the main web app."""

    def get(self, request):
        """Save host/``state``/``next_path`` in the session and redirect to the GitLab authorize URL."""
