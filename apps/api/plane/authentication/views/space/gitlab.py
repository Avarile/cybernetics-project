...
"""
GitLab OAuth views for the public space frontend (``/auth/spaces/gitlab/`` and
``/auth/spaces/gitlab/callback/``).

The initiate view stores the frontend host and a random ``state`` (CSRF protection) in
the session, then redirects to GitLab. The callback logs the user in (no invitation
...
class GitLabOauthInitiateSpaceEndpoint(View):
    """Start the GitLab OAuth flow for the public space frontend."""

    def get(self, request):
        """Save host and ``state`` in the session and redirect to the GitLab authorize URL."""
...
class GitLabCallbackSpaceEndpoint(View):
    """Handle the GitLab redirect back to Plane and log the user in."""
