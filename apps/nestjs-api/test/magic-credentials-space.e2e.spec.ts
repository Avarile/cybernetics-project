...
// Space-audience variant of magic-credentials.e2e.spec.ts (views/space/magic.py): same form-POST
// -> 302-redirect protocol, CSRF-guarded, only the redirect target (SPACE_BASE_URL) differs.
// plane_test's seed writes ENABLE_MAGIC_LINK_LOGIN='0' + EMAIL_HOST='' -- this suite upserts both
// to enabled values for its own duration and always restores them in `afterAll`, same discipline
// as magic-credentials.e2e.spec.ts.
...
function email(tag: string) {
  return `magic-cred-space-${tag}-${randomUUID()}@test.dev`;
}
...
describe("Magic credentials (spaces) — sign-in/up (redirect + CSRF parity)", () => {
  it("magic-sign-in: correct code + csrf -> 302 to SPACE_BASE_URL + session cookie", async () => {
  ...
  it("magic-sign-in: POST without a csrf token -> 403", async () => {
    ...
    const res = await http.post("/auth/spaces/magic-sign-in").type("form").send({ email: addr, code: "123456" });
    expect(res.status).toBe(403);
  });

  it("magic-sign-up: new email + correct code -> creates a user with is_password_autoset=true, 302 to SPACE_BASE_URL + cookie", async () => {
